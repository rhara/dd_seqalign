# dd_seqalign — ひとつのアクセッション番号から、すべての構造へ——配列カバレッジとアクティブサイトRMSDで比較する
...
再利用可能なパッケージとして設計されている（`dd_prep`/`dd_afpocket`/`dd_viewer`
などと同じ方針——以下の例はすべてヒトCDK1、UniProt `P06493` を用いるが、
どのアクセッション番号でも動作する）。完全に単独で動作するパッケージ——
構造ダウンロードとHETATM分類（`pdbio.py`）、fpocketベースのポケット検出
（`pocket.py`）に必要な部分はこのリポジトリ内に直接vendor（複製）されて
おり、インストール時に`dd_prep`/`dd_afpocket`へ依存しない。

- **Fetch（`dd_seqalign-fetch`）**: `list_pdb_ids_for_uniprot`（RCSB Search
  API）が、そのアクセッション番号に紐付く全PDBエントリを見つけ、各エントリ
  は`fetch.download_pdb`経由で、AlphaFold DBモデルは
  `fetch.download_afdb`経由で、正準配列はUniProt REST API経由で
...
- **Align（`dd_seqalign-align`）**: 取得済みの全構造について、各鎖の配列を抽出
  ...
  リガンド近傍の残基、`--site-mode pocket`: `pocket.py`経由のfpocketの
  ...
  スキップされる。
- **Run（`dd_seqalign-run`）**: fetchとalignを1ステップで実行する。
...
入らないためconda-forge必須。Pythonバインディングは存在せず、
`pocket.py`からサブプロセスとして呼び出される）が必要。`dd_viewer`
（`[app]` extraの3Dビューに必要）は、廃止された独立プロジェクト
`dd_viewer`から無改変のまま吸収し、このレポジトリ内に直接ベンダーして
いる（`dd_viewer/`）ため、別途インストール不要——このパッケージが必要と
する`dd_prep`/`dd_afpocket`の一部（`pdbio.py`、`pocket.py`）も同様に
ベンダーされているため、姉妹`dd_*`パッケージを別途インストールする必要
はない。`dd_seqalign`は専用のconda envを持つ（他の姉妹プロジェクトが
使っている共有`dd` envではない）:
...
これにより3つのコンソールコマンド、`dd_seqalign-fetch`、`dd_seqalign-align`、
`dd_seqalign-run`がインストールされる。
...
- **本格的なMSAツールを使わない理由**: mafft/clustaloは`dd_seqalign` envに無く、
...
- **3Dタブで`dd_viewer`の`view3d`コンポーネントを再利用する**: プレーンな
  ...
  表示する前に直前のカメラ位置を再適用する。同じ仕組みを`dd_seqalign`側で
  再実装するのではなく直接再利用しているのが、`dd_viewer`をこのレポジトリに
  ベンダーしている理由である（上記インストール参照）。
...
- 目的蛋白質が短い無関係なペプチド断片のみを提供する共結晶構造
  ...
  `dd_seqalign-align`は誤ってフィットさせるのではなく、これらを正しくスキップ

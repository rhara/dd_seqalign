...
入らないためconda-forge必須）、`dd_prep`/`dd_af`パッケージ（`[app]`
extraでは`dd_viewer`も）が必要。`dd_seqalign`は専用のconda envを持つ
（他の姉妹プロジェクトが使っている共有`dd` envではない）:
...
cd dd_viewer && pip install --no-deps -e . && cd .. # if not already installed ([app] extra のみで使用、後述)

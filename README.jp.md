...
分類）と`dd_afpocket`（fpocketベースのポケット検出）をそれぞれ再実装せず直接
再利用する。
...
- **Align（`dd_seqalign-align`）**: 取得済みの全構造について、各鎖の配列を抽出
  ...
  リガンド近傍の残基、`--site-mode pocket`: `dd_afpocket.pocket`経由のfpocketの
...
入らないためconda-forge必須）、`dd_prep`/`dd_afpocket`パッケージが必要。
`dd_viewer`（`[app]` extraの3Dビューに必要）は、廃止された独立プロジェクト
`dd_viewer`から無改変のまま吸収し、このレポジトリ内に直接ベンダーして
いる（`dd_viewer/`）ため、別途インストール不要。`dd_seqalign`は専用の
conda envを持つ（他の姉妹プロジェクトが使っている共有`dd` envではない）:

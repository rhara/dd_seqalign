...
- **App（`streamlit run app.py -- --report-dir DIR`）**: 3タブ構成——
  ...
  細いワイヤーフレーム（`line`表示）に切り替える——3Dmol.jsの`within`+
  `byres`セレクタでサイト残基周辺の原子を残基単位で抽出し、それ以外は
  完全に非表示にする。この蛋白ワイヤーは構造ごとの色を白方向に薄めた色で
  描画し、リガンド（表示している場合）は薄めない元の色のまま描くことで、
  リガンドが背景の蛋白より視覚的に際立つようにしている。さらに蛋白ワイヤー
  とリガンドの両方とも、炭素だけを（薄めた／通常の）構造色に、それ以外の
  元素（O/N/S/Pやハロゲンなど）はRasMol系の標準元素色に塗り分ける
  カラースキームを使う——3Dmol.js組み込みの`"*Carbon"`スキーム（例:
  `"yellowCarbon"`）と同じ仕組みだが、named CSS色に限定されずどんな16進
  色にも使えるよう自前で実装している。
